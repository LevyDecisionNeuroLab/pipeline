...
folder = glob.glob('/media/Data/PTSD_Reversal_2018/scan_data/raw/eyetracking/rv*/*')
...
for sub_idx in range(len(folder)):
    print(sub_idx)
    sub_folder = folder[sub_idx].split('rv')
...
for sub_idx in range(len(sub_id)): 
    ...
    for i in folder:
        ...
        if i.find('rv%s' %subject_id)!=-1:
            print (i)
            # take session count from folder name
            session_count = 1 #i.split('D')[3].split('_')[0]
            #print(i)
...
#%% convert
output_dir = '/media/Data/RV_BidsConversion/' 
...
#%% Need to remove one dash line from rest tasks
fileList = glob.glob('/media/Data/RV_BidsConversion/sub-*/ses-1/func/sub-*_ses-1_task--rest_bold.*')
for file in fileList:
    print(file)
    a = file.split('--')
    os.rename(file , a[0] + '-'+ a[1])
    
#%% Renaming task name as its empty now
fileList = glob.glob('/media/Data/RV_BidsConversion/sub-*/ses-1/func/sub-*_ses-1_task-_bold.*')
for file in fileList:
    print(file)
    a = file.split('task-')
    os.rename(file , a[0] + 'task-rv'+ a[1])
